...
@WebServlet({"/", "/login", "/autentica", "/logoff" })
public class UsuarioControle extends HttpServlet {
	...
	private void autentica(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		if (!request.getMethod().equalsIgnoreCase("post")) {
			...
			if (usuario != null && senha != null && senha.equals(usuario.getSenha())) {
				...
				HttpSession session = request.getSession();
				session.setAttribute("login_erro", "Usuario e senha inválido");
